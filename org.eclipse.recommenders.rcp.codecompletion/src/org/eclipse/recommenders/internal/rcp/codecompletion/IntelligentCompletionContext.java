...
public class IntelligentCompletionContext implements
		IIntelligentCompletionContext {
	...
	public IntelligentCompletionContext(
			final JavaContentAssistInvocationContext jdtCtx,
			final JavaElementResolver resolver) {
		ensureIsNotNull(jdtCtx);
		ensureIsNotNull(resolver);
		jdtCompletionContext = jdtCtx;
		this.resolver = resolver;
		jdtCompilationUnit = (org.eclipse.jdt.internal.core.CompilationUnit) jdtCtx
				.getCompilationUnit();
	...
	private void initializeCompletionPrefixToken() {
		try {
			...
			IntelligentCompletionPlugin.logError(x,
					"Computing token from active editor failed.");
			token = "";
		}
	}

	private void initializeRequestor() {
		completionRequestor = new IntelligentCompletionRequestor(
				jdtCompilationUnit);
	}

	private void initializeCompletionEngine() {
		try {
			final JavaProject project = (JavaProject) jdtCompilationUnit
					.getJavaProject();
			final WorkingCopyOwner owner = jdtCompilationUnit.getOwner();
			final SearchableEnvironment s = project
					.newSearchableNameEnvironment(owner);
			completionEngine = new CompletionEngine(s, completionRequestor,
					project.getOptions(true), project, owner,
	...
	private void performCodeCompletion() {
		...
		completionEngine.complete(compilerCu, getInvocationOffset(), 0,
	...
	private void findCompletionNode() {
		...
		if (completionParser.compilationUnit != null) {
			...
			compilationUnit.traverse(astCompletionNodeFinder,
	...
	private CompilationUnitDeclaration findCompilationUnit(
			final AbstractMethodDeclaration methodContext) {
	...
	@Override
	public IJavaCompletionProposal toJavaCompletionProposal(
	...
	private IJavaElement findEnclosingElement() {
		final CompletionContext coreContext = completionRequestor
	...
	private IMethod findFirstDeclaration(IMethod method) {
		...
		try {
			final MethodOverrideTester methodOverrideTester = SuperTypeHierarchyCache
					.getMethodOverrideTester(method.getDeclaringType());
	...
	private ITypeName getSuperclassOfEnclosingType() {
		try {
			...
			final ITypeHierarchy typeHierarchy = SuperTypeHierarchyCache
					.getTypeHierarchy(enclosingType);
	...
	public ITypeName getReceiverType() {
		return CompilerBindings
				.toTypeName(astCompletionNodeFinder.receiverType);
	}

	@Override
	public ITypeName getExpectedType() {
		return CompilerBindings
				.toTypeName(astCompletionNodeFinder.expectedReturnType);
	}
	...
	public Variable getVariable() {
		if (isReceiverImplicitThis()) {
			return Variable.create("this", getSuperclassOfEnclosingType(),
					getEnclosingMethod());
		}
		if (getReceiverName() != null && getReceiverType() != null) {
			return Variable.create(getReceiverName(), getReceiverType(),
					getEnclosingMethod());
		}
	...
	private FieldDeclaration findMatchingFieldDeclaration(
	...
	public String toString() {
		return ToStringBuilder.reflectionToString(astCompletionNodeFinder,
