...
public class ParameterCallsitesTracingTest {
	...
	public void setup() {
		...
		sut = new CallGraphMethodAnalyzer(
				new Provider<InstanceCallGraphBuilder>() {
					@Override
	...
	@Test
	public void testCallsToSeveralSameTypeLocalsUsingDelegateMethod()
			throws Exception {
		// setup
	...
	private void exerciseSUTAndFilterValues(
			final RecommendersEntrypoint entrypoint) {
	...
	private void filterValuesWithEmtpyParameterCallsites() {
		for (final Iterator<ObjectInstanceKey> it = output.objects.iterator(); it
				.hasNext();) {
	...
	private void checkParameterCallSitesCount(final MethodDeclaration output,
		...
		final List<Integer> expectedReceiverCallsitesCount = Lists
				.newLinkedList(Ints.asList(pExpectedParameterCallsitesCount));
		for (final ObjectInstanceKey value : output.objects) {
			final Integer actualCount = value.parameterCallSites.size();
			final boolean foundCountInExpected = expectedReceiverCallsitesCount
					.remove(actualCount);
			if (!foundCountInExpected) {
				final String failMessage = format(
						"actual count '%d' for '%s' could not be found (anymore?)",
						actualCount, value);
				fail(failMessage);
			}
		}
		if (!expectedReceiverCallsitesCount.isEmpty()) {
			final String failMessage = format(
					"expected '%d' more traces with counts '%s'.",
					expectedReceiverCallsitesCount.size(),
					expectedReceiverCallsitesCount);
			fail(failMessage);
		}
	}
}
