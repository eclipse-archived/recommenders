...
public final class CodeBuilderTest {
    ...
    public void testBuildCode() throws JavaModelException {
        ...
        methods.add(TestUtils.getDefaultConstructorCall().getInvokedMethod());
        methods.add(TestUtils.getDefaultMethodCall().getInvokedMethod());
        methods.add(TestUtils.getDefaultReturningMethodCall().getInvokedMethod());
        methods.add(TestUtils
        ...
        Assert.assertEquals(
                expected,
                codeBuilder.buildCode(methods, TestUtils.getMockedTargetVariable("someWidget")).replaceAll(
                        "[\\s]+", " "));
    }
    ...
    public void testBuildCodeNoMethods() throws JavaModelException {
        codeBuilder.buildCode(new ArrayList<IMethodName>(), TestUtils.getMockedTargetVariable(""));
    }
}
