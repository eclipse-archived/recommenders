...
public class JavaSnippetMatchEnvironment extends SnippetMatchEnvironment {
                        ...
                        if (param.getMinorType() != null && !param.getMinorType().isEmpty()) {
                            ...
                            if (param.getMinorType().equals("$loopable")) {
                                ...
                                try {
                                    ...
                                    Class<?> childClass = Class.forName(childEffect.getMinorType(), false, null);
                            ...
                            try {
                                ...
                                Class<?> parentClass = Class.forName(param.getMinorType(), false, null);
                                Class<?> childClass = Class.forName(childEffect.getMinorType(), false, null);
    ...
    protected void applyResult(Object result, Effect effect) throws Exception {
        ...
        ReplaceEdit markerCodeEdit = new ReplaceEdit(destStart, destEnd - destStart, formattedCode);

        // This version of the text edit has the special markers removed, and is the one added to the final edit.
        ReplaceEdit codeEdit = new ReplaceEdit(destStart, destEnd - destStart, formattedCode.replace(CURSOR_MARKER, "")
                .replace(END_MARKER, ""));
        ...
        for (startLinesIn = 0; startLinesIn < doc.getNumberOfLines(); startLinesIn++) {
            ...
            String oldLine = doc.get(doc.getLineOffset(startLinesIn), doc.getLineLength(startLinesIn));

            String newLine = importEditDoc.get(importEditDoc.getLineOffset(startLinesIn),
        ...
        for (endLinesIn = 0; endLinesIn < doc.getNumberOfLines(); endLinesIn++) {
            ...
            String oldLine = doc.get(doc.getLineOffset(doc.getNumberOfLines() - endLinesIn - 1),
        ...
        if (startLinesIn <= doc.getNumberOfLines() - endLinesIn) {
            ...
            ReplaceEdit helperImportEdit = new ReplaceEdit(destStart, destEnd - destStart, importEditDoc.get(srcStart,
                    srcEnd - srcStart));
        ...
        if (confirmed) {
            ...
            int resultsEnd = doc.getLineOffset(lastLine + searchBoxHeight
                    / (styledText.getLineHeight() + styledText.getLineSpacing()));
    ...
    @Override
    protected String evaluateFormulaSnippetNode(FormulaSnippetNode formNode, HashMap<String, String> variables,
    ...
    @Override
    protected String evaluateTextSnippetNode(TextSnippetNode textNode, HashMap<String, String> variables,
            EffectMatchNode effectNode) {
    ...
    private String getFreeHelperName(String desiredName, String desiredId, ICompilationUnit parentElement)
            throws Exception {
