...
class CallCompletionProposalComputerSmokeTest {
    ...
    def private test(CharSequence code, int numberOfExpectedProposals) {
        ...
        assertNotNull(ast)
        val CallsCompletionProposalComputer sut = new CallsCompletionProposalComputer(new ModelStoreMock(),
            new JavaElementResolver(), new RecommendersCompletionContextFactoryMock(),
            CallsPreferenceStoreMock::create())
    ...
    def static Pair<List<IJavaCompletionProposal>, CallsCompletionProposalComputer> exercise(CharSequence code) {
        ...
        val CallsCompletionProposalComputer sut = new CallsCompletionProposalComputer(new ModelStoreMock(),
            new JavaElementResolver(), new RecommendersCompletionContextFactoryMock(),
            CallsPreferenceStoreMock::create())
        val proposals = sut.computeCompletionProposals(new JavaContentAssistContextMock(cu, struct.second.head),
