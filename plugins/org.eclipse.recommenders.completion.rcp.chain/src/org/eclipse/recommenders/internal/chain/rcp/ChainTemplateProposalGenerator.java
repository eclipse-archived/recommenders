...
import org.eclipse.recommenders.internal.chain.rcp.algorithm.FieldChainElement;
import org.eclipse.recommenders.internal.chain.rcp.algorithm.IChainElement;
import org.eclipse.recommenders.internal.chain.rcp.algorithm.InheritanceHierarchyCache;
import org.eclipse.recommenders.internal.chain.rcp.algorithm.MethodChainElement;
...
public class ChainTemplateProposalGenerator {
    ...
    private JavaContext computeJavaContext() {
        ...
        final JavaContext ctx = new JavaContext(templateContextType, document, offsetToEquals, 0,
                jctx.getCompilationUnit());
    ...
    private void computeAndAddProposal(final int proposalNo, final ChainTemplateProposal chainProposal,
    ...
    private void computeCastingForName(final ChainTemplateProposal chainProposal, final StringBuilder code) {
        if (chainProposal.needsCast()) {
            code.insert(
                    0,
                    String.format("(%s) ", chainProposal.getCastingType().getReference().getName().getClassName()
                            .toString()));
        }
    }
    ...
    private String makePartName(final IChainElement part, final int chainPosition) {
        ...
        case FIELD:
            result = checkForThisQualifier((FieldChainElement) part, chainPosition) ? "this." + part.getCompletion()
                    : part.getCompletion();
            break;
        case METHOD:
    ...
    private String makePartCode(final IChainElement part, final int chainPosition) throws JavaModelException {
        ...
        case METHOD:
            ...
            final StringBuilder methodCode = new StringBuilder().append(part.getCompletion()).append(
                    Signature.C_PARAM_START);
