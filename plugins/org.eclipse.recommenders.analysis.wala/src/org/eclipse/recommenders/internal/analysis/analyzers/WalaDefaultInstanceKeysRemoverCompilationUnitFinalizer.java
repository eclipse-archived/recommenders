...
public class WalaDefaultInstanceKeysRemoverCompilationUnitFinalizer implements ICompilationUnitFinalizer {
    ...
    public void finalizeTypeDeclaration(final TypeDeclaration type) {
        for (final MethodDeclaration method : type.methods) {
            ...
            for (final TypeDeclaration nestedType : method.nestedTypes) {
                finalizeTypeDeclaration(nestedType);
            }
        }
    }
    ...
    public void finalizeMethodDeclaration(final MethodDeclaration method) {
        for (final Iterator<ObjectInstanceKey> it = method.objects.iterator(); it.hasNext();) {
            ...
            if (isExceptionInInitializer(value) || isOutOfMemoryError(value) || isNullPointerException(value)) {
                if (value.parameterCallSites.isEmpty() && value.receiverCallSites.isEmpty()) {
                    it.remove();
                }
            }
        }
    ...
    private boolean isNullPointerException(final ObjectInstanceKey value) {
        return value.type == VmTypeName.JavaLangNullPointerException;
    }

    private boolean isOutOfMemoryError(final ObjectInstanceKey value) {
        return value.type == VmTypeName.JavaLangOutOfMemoryError;
    }

    private boolean isExceptionInInitializer(final ObjectInstanceKey value) {
        return value.type == VmTypeName.JavaLangExceptionInInitializerError;
    }
}
