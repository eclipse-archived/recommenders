...
class SearchThread extends PostThread {
    ...
    public SearchThread(RemoteMatchClient client, MatchEnvironment env, String query, long waitTime,
            ISearchListener listener) {

        super(client, RemoteMatchClient.SEARCH_URL);
        this.env = env;
        this.query = query;
    ...
    private String parseResults(InputStream input) {
        ...
        for (int i = 0; i < matchNodes.getLength(); i++) {
            ...
            Element rootNodeNode = (Element) matchNode.getElementsByTagName("node").item(0);
