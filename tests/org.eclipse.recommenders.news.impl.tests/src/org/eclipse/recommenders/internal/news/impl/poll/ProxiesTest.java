...
public class ProxiesTest {
    ...
    public void testUserDomain() {
        assertThat(Proxies.getUserDomain("mydomain\\\\user"), is(equalTo("mydomain")));

        System.setProperty(PROP_HTTP_AUTH_NTLM_DOMAIN, DOMAIN_PROPERTIES);

        assertThat(Proxies.getUserDomain(DOMAIN_PROPERTIES), is(equalTo(DOMAIN_PROPERTIES)));
    }

    @Test
    public void testWorkstation() {
        assertNotNull(Proxies.getWorkstation());
    }

    @Test
    public void testUserName() {
        assertEquals("user", Proxies.getUserName("\\\\user"));
        assertEquals("user2", Proxies.getUserName("domain\\\\user2"));
        assertEquals("user3", Proxies.getUserName("user3"));
    }
    ...
    public void testGetProxyHost() throws URISyntaxException {
        ...
        assertThat(Proxies.getProxyHost(service, uri), is(equalTo(new HttpHost(host, PORT))));
    }
    ...
    public void testProxyAuthentication() throws URISyntaxException, IOException {
        ...
        when(executor.auth(new AuthScope(new HttpHost(host, PORT), AuthScope.ANY_REALM, "ntlm"),
                new NTCredentials(USER, PASSWORD, Proxies.getWorkstation(), Proxies.getUserDomain(USER))))
                        .thenReturn(executor);
