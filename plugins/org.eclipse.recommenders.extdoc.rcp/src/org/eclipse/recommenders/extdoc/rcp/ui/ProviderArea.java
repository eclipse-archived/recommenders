...
public class ProviderArea {
    ...
    private GridLayoutFactory layoutFactory = GridLayoutFactory.fillDefaults().spacing(0,0);
    private GridDataFactory layoutDataFactory = GridDataFactory.fillDefaults().grab(true, false);

    public ProviderArea(Provider provider) {
    ...
    private void createTitleArea() {
        ...
        String providerName = provider.getDescription().getName();
        Image providerImage = provider.getDescription().getImage();
    ...
    public void setStatus(String statusMessage) {
        disposeChildren(status);
        Link link = new Link(status, SWT.NONE);
